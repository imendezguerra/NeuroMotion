...
if __name__ == '__main__':
    ...
    parser.add_argument('--dur_ramp_s', default=1, type=float, help='Duration in seconds of the ballistic ramp')
    parser.add_argument('--dur_rest_s', default=3, type=float, help='Duration in seconds of the rest period')
    parser.add_argument('--reps', default=30, type=int, help='Number of ballistic repetitions')
    ...
    parser.add_argument('--path_save', default='./res/ballistic', type=str, help='path to save the simulated contraction')
    ...
    # ---------------------------------
    muscle_force = np.tile(
        np.concatenate([
        np.zeros(round(args.dur_rest_s * fs)),
        np.linspace(0, args.mvc_level, round(args.dur_ramp_s * fs))
        ]), args.reps
    )
    muscle_force = np.concatenate([muscle_force, np.zeros(fs)]) # Add an extra second at the end
    dur_s = len(muscle_force) / fs
    angle_profile = np.ones(round(dur_s * fs)) * args.angle
    timestamps = np.linspace(0, dur_s, round(dur_s * fs))
    ...
    file_save = os.path.join( args.path_save, f'semg_{args.muscle}_ballistic_prepro_{args.mvc_level}mvc_{args.snr_level}dB_bs{args.iter}.hdf5')
