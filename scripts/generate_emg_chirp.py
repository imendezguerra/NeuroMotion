...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_save', default='./res/chirp', type=str, help='path to save the simulated contraction')
    ...
    # ---------------------------------
    sin_dur_s = 10
    x = np.linspace(0, sin_dur_s, round(sin_dur_s * fs))
    muscle_force = np.concatenate([
        np.sin(2 * np.pi * 1 * x), # 1 Hz
        np.sin(2 * np.pi * 2 * x), # 2 Hz
        np.sin(2 * np.pi * 3 * x), # 3 Hz
    ]) * args.mvc_level/2 + args.mvc_level/2
    muscle_force = np.concatenate([
        np.ones(fs) * args.mvc_level / 2, 
        muscle_force, 
        np.ones(fs) * args.mvc_level / 2
        ])
    ...
    file_save = os.path.join( args.path_save, f'semg_{args.muscle}_chirp_prepro_{args.mvc_level}mvc_{args.snr_level}dB_bs{args.iter}.hdf5')
